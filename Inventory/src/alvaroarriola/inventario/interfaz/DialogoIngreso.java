...
public class DialogoIngreso extends JDialog implements ActionListener
{
	...
	public DialogoIngreso(InterfazPrincipal ip, int nentrada)
	{
		super(ip,true);
		principal = ip;
		NumeroDeEntrada = nentrada;
	...
	public void panelTipoIngreso()
	{
		...
		//ptipo.setBorder(BorderFactory.createTitledBorder("Tipo de salida"));
		
		uno = new JRadioButton("Entrada");
		dos = new JRadioButton("Hoja de Salida");
		tres = new JRadioButton("Devolucion");
	...
	private void cerrarDialogo() throws Exception
	{
		...
		Ingreso in = new Ingreso(nomProducto,tamaño,cantidad,linea);
		
		principal.inventario.entradas.ingresoMercaderia(tipoIngreso, in, dia, mes, año, NumeroDeEntrada);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getSource() == Aceptar)
		{
			try
			{
				...
				principal.inventario.setNumeroEntrada(NumeroDeEntrada+1);
			}
			catch(Exception exc)
			{
		...
		if(e.getSource() == Continuar)
		{
			try
			{
				...
				DialogoIngreso dIngreso = new DialogoIngreso(principal, NumeroDeEntrada);
